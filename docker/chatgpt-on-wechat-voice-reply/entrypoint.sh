...
CHARACTER_DESC_PRIVATE=${CHARACTER_DESC_PRIVATE:-""}
...
if [ "$CHARACTER_DESC_PRIVATE" != "" ] ; then
    sed -i "s/\"character_desc_private\".*,$/\"character_desc\": \"$CHARACTER_DESC_PRIVATE\",/" $CHATGPT_ON_WECHAT_CONFIG_PATH
fi
